...
const Navigation = () => {
  ...
  const burgerClass = () => {
    if (scrolled || forceOverlay) return "bg-white/90 text-foreground border";
    return "bg-white/10 text-white";
  };

  const mobileLinkClass = (active: boolean) =>
    active ? "bg-foreground/5 text-foreground font-semibold" : "text-foreground/90 hover:text-primary";
